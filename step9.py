...
from mal_types import (
    MalType, MalSymbol, MalList, MalVector, MalHashmap, MalNil, MalBoolean, MalFunction, MalAtom,
    MalString, MalIterable,
)
...
TRY_SYMBOL = MalSymbol('try*')

# functions
CONCAT_SYMBOL = MalSymbol('concat')
CONS_SYMBOL = MalSymbol('cons')
VEC_SYMBOL = MalSymbol('vec')
THROW_SYMBOL = MalSymbol('throw')
MAP_SYMBOL = MalSymbol('map')
APPLY_SYMBOL = MalSymbol('apply')
    ...
    while True:
        ...
        if ast.value[0] == DEF_SYMBOL:
            try:
                ...
                raise MalString('def! syntax is (def! /symbol/ /value/)')
            env.set(symbol, EVAL(value, env))
            return MalNil(None)

        # create environment with temporary bindings
        elif ast.value[0] == LET_SYMBOL:
            let_error = MalString('let* syntax is (let* /list_of definitions/ /list_of_instructions/)')
            new_env = Env(env)
            try:
        ...
        # if else
            ...
            try:
                ...
                raise MalString('if syntax is (if /condition/ /true_branch/ /false_branch/)')
        ...
        # function construction (without binding to env)
            ...
            try:
                ...
                raise MalString('fn* syntax us (fn* /arguments/ /function_body/)')
        ...
        # quasiquoting element
            ...
            try:
                ...
                raise MalString('defmacro! syntax is (def! /symbol/ /function_body/)')
            fn = MalFunction(body, binds, env, None, True)  # fn.fn is set to None. Check in step 9 is it ok
            env.set(symbol, fn)
            return MalNil(None)

        elif ast.value[0] == TRY_SYMBOL:
            print('allal')
            op, try_branch, catch = ast.value
            try:
...
def deref(mal_type: MalType) -> MalType:
    if not isinstance(mal_type, MalAtom):
        raise MalString(f'Error: {mal_type.mal_repr(True)} is not an atom')
...
def throw(exc: MalType):
    raise exc


def mal_map(fn, mal_iter: MalIterable):
    if isinstance(fn, MalFunction):
        fn = fn.fn
    return MalList(list(map(fn, mal_iter.value)))


@log_function
def _flatten(arg: MalType) -> Union[MalIterable, MalType]:
    if not isinstance(arg, MalIterable):
        return arg
    elif not arg:
        return MalList([])
    elif not isinstance(arg.value[0], MalIterable):
        return MalList([
            arg.value[0],
            *_flatten(MalList(arg.value[1:])).value
        ])
    return MalList([
        *_flatten(arg.value[0]).value,
        *_flatten(MalList(arg.value[1:])).value
    ])


def apply_(fn, *args):
    args = _flatten(MalList(list(args)))
...
def setup_fns():
    ...
    repl_env.set(THROW_SYMBOL, throw)
    repl_env.set(MAP_SYMBOL, mal_map)
    repl_env.set(APPLY_SYMBOL, apply_)
